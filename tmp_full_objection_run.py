...
async def run_full_objection_batch():
    objection_runs = [
        ("james", "james_objections"),
        ("morgan", "morgan_objections"),
        ("sarah-netic", "sarah_objections"),
        ("luke", "luke_emergency"),
        ("dani", "dani_objections"),
        ("amy", "amy_objections"),
        ("claire", "claire_objections")
    ]
    ...
    for agent, pack in objection_runs:
        print(f"🚀 Running Full Objection Phase: {agent} -> {pack}")
        tool = XAgentEvalTool()
        tool.batch_id = f"full_objection_{agent}"
        ...
        if await tool.prepare(context, inputs):
            ...
            print(f"❌ {agent} failed preparation: {tool.result.data if tool.result else 'No result'}")
            results[agent] = "PREP_FAILED"

    with open("full_objection_results.json", "w") as f:
        json.dump(results, f, indent=2)

if __name__ == "__main__":
    asyncio.run(run_full_objection_batch())
