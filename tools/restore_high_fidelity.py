...
ROOT_DIR = os.getcwd()
BACKUP_DIR = os.path.join(ROOT_DIR, 'back up system prompts')
AGENTS_PATH = os.path.join(ROOT_DIR, 'config', 'agents.yaml')

# Map backups to the Hub slugs
FILE_MAP = {
    'morgan': 'Morgan GoDeskless FST.txt',
    'sarah-netic': 'Sarah Netic SDR.txt',
    'dani': 'Dani X Agent Director.txt',
    'amy': 'Amy Insight SDR.txt',
    'james': 'James Knowles Law Firm.txt',
    'luke': 'Luke After Hours Vet Triage.txt',
    'claire': 'Claire Flemings OpenTable.txt',
    'taylor': 'Taylor SDR.txt', # Note: This will be injected with Canyon Ridge instead
    'michael': 'Michael Real Estate Intake Specialist for Fulton Homes.txt'
}

# The User's NEW Taylor Prompt (Canyon Ridge Solutions)
TAYLOR_CANYON_RIDGE = """You are Taylor, a senior SDR-style representative for Canyon Ridge Solutions.
...
def rebuild_baseline():
    print("🚀 Rebuilding agents.yaml from backup prompts...")
    ...
    for slug in slug_order:
        ...
        # 1. Handle Taylor Injetion
        if slug == 'taylor':
            persona = TAYLOR_CANYON_RIDGE
            print("    ✅ Injected NEW Canyon Ridge Taylor Persona.")
        else:
            ...
            if os.path.exists(path):
                ...
                print(f"    ✅ Extracted {len(persona.splitlines())} lines from {filename}.")
            else:
        ...
        # We use a standard template to ensure metadata consistency
        block = (
            f"- slug: {slug}\n"
    ...
    print(f"✨ SUCCESS: agents.yaml restored to high-fidelity (~4600 lines)")
