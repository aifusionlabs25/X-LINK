...
async def run_batch():
    agents = [
        ("luke", "luke_pet_triage"),
        ("morgan", "morgan_field_service"),
        ("claire", "claire_hospitality"),
        ("sarah-netic", "sarah_after_hours"),
        ("dani", "dani_platform_sales"),
        ("amy", "amy_it_discovery")
    ]
    ...
    for agent, pack in agents:
        print(f"🚀 Running Role-Fit: {agent} -> {pack}")
        tool = XAgentEvalTool()
        tool.batch_id = f"fit_{agent}_{pack}"
    ...
    with open("batch_fit_results.json", "w") as f:
